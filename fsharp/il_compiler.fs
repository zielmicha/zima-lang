...
type ClrVar = { clrType: ClrType; ref: LocalBuilder }

type ClrJoin = { label: Label; args: list<ClrVar> }
...
let allocLocal env (name: AVarName) typ =
    if toClrType typ = typeof<System.Void>
    then failwithf "cannot allocate void variable"

    let localBuilder =
        env.ilGenerator.DeclareLocal(toClrType typ)

    let clrVar = { clrType = typ; ref = localBuilder }

    clrVar,
    { env with
          Env.vars = Map.add name clrVar env.vars }
...
let loadVarConverted env (name: AVarName) (targetType: ClrType) =
    ...
    | Native t1, Native t2 ->
        ...
        if t1 <> t2
        then failwithf "attempting to convert %A to %A" t1 t2
    | AnyObject, Native t ->
        if t = typeof<System.Void>
        then failwithf "cannot store void value"
        else if t.IsValueType
        then env.ilGenerator.Emit(OpCodes.Unbox_Any, t)
        else env.ilGenerator.Emit(OpCodes.Castclass, t)
    | Native t, AnyObject ->
        if t = typeof<System.Void> then failwithf "cannot load void value"
        if t.IsValueType then env.ilGenerator.Emit(OpCodes.Box, t)
...
let compileAtomicExpr env (expr: AtomicExpr<AVarName>) =
    ...
    | AtomicExpr.MakeRecord (recordId, values) ->
        ...
        let ctrs =
            typeof<Roboot.Runtime.DynObject>.GetConstructors()
    ...
    | AtomicExpr.RaiseUnexpectedVariant ->
        env.ilGenerator.EmitCall
            (OpCodes.Call,
             typeof<Roboot.Runtime.Util>
                 .GetMethod("RaiseUnexpectedVariant"),
             null)
        // RaiseUnexpectedVariant should actually return void
    ...
    | AtomicExpr.ClrCall (objType, name, argTypes, returnType, virt, args) ->
        let methodInfo =
            objType.GetMethod(name, (Array.ofList argTypes))

        List.zip argTypes args
        |> List.iter (fun (t, arg) -> loadVarConverted env arg (Native t))

        env.ilGenerator.EmitCall((if virt then OpCodes.Callvirt else OpCodes.Call), methodInfo, null)
        let returnType = convertVoidOnStackToUnit env returnType
        Native returnType

let convertVoidOnStackToUnit env returnType =
    if returnType = typeof<System.Void> then
        env.ilGenerator.Emit(OpCodes.Ldnull)
        typeof<unit>
    else
        returnType
...
let compileExpr env (expr: AExpr) =
    match expr with
    | AExpr.Atomic expr -> compileAtomicExpr env expr
    | Lambda (_, _) -> failwith "il_compiler encountered unlifted lambda"
    | LiftedLambda (context, contextArg, arg, body) ->
        let (ilGenerator, methodInfo) =
            env.emitHelperMethod ("closure", typeof<obj>, [ typeof<obj>; typeof<obj> ])

        let req =
            { CompilationRequest.body = ([ contextArg; arg ], body)
              argType = AnyObject
...
let compilePatternMatch env value (patternId, patternFields) ifMatches ifFails =
    ...
    List.indexed patternFields
    |> List.iter (fun (i, (fieldId, _)) ->
        env.ilGenerator.Emit(OpCodes.Dup)
        env.ilGenerator.Emit(OpCodes.Ldc_I4, i)
    ...
    let env =
        List.fold f env (List.indexed patternFields)
...
let compileNode env (body: ANode) =
    ...
    | ANode.LetJoin { AJoin.name = name; args = args; jumpBody = jumpBody; body = body } ->
        ...
        let argVars =
            List.map (fun arg ->
                let (var, newEnv) = allocLocal env arg AnyObject
                envForJump <- newEnv
                var) args
        ...
        let envForBody =
            { env with
                  labels = Map.add name clrJoin env.labels }
    ...
    | ANode.JumpToJoin (name, vars) ->
        ...
        List.zip vars args
        |> List.iter (fun (src, dst) -> copyVar env src dst)

        env.ilGenerator.Emit(OpCodes.Br, label)
    | ANode.PatternMatch (value, patternShape, ifMatches, ifFails) ->
        match patternShape with
        | ANF.Record (id, fields) -> compilePatternMatch env value (id, fields) ifMatches ifFails
        | ANF.IsTrue t -> compileIfTrue env value t ifMatches ifFails
    | ANode.Return varName ->
        loadVarConverted env varName AnyObject
        env.ilGenerator.Emit(OpCodes.Ret)

let compile { CompilationRequest.body = (args, body); argType = argType; returnType = returnType;
              mainMethod = (ilGenerator, _); emitHelperMethod = emitHelperMethod; findSigByGlobalId = findSigByGlobalId } =
    let env =
        { Env.vars = Map.empty
          labels = Map.empty
          ilGenerator = ilGenerator
    ...
    let env =
        List.fold loadArg env (List.indexed args)
