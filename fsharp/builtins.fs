    ...
    Atomic
        (MakeRecord
            (kind,
             (args.attrs
              |> Map.toList
              |> List.map (fun (key, value) -> (key, recur value)))))

let buildArgs recur args = buildRecord recur Ids.args args

let expandCallLocal recur args =
    match splitHead args with
    | Some (head, args) -> Atomic(CallLocal(recur head, buildArgs recur args))
...
let expandRecord recur args =
    ...
    | Some (head, args) ->
        match head with
        | GlobalId id -> buildRecord recur id args
...
let expandUnit recur args = Atomic(MakeRecord(Ids.unit, []))

let clrTypeByName name =
    match System.Type.GetType name with
    | null ->
        failwithf
            "no such type %A (use assembly qualified name, like %A)"
            name
            (typeof<System.Console>.AssemblyQualifiedName)
    | t -> t

let clrTypeFromAst ast =
    match ast with
    | LiteralString s -> clrTypeByName s
    | Composite (OnlyPositional (name :: genericArgs)) ->
        let baseType = clrTypeFromAst name
        let args = genericArgs |> List.map clrTypeFromAst
        baseType.MakeGenericType(args |> Array.ofList)
    | _ -> failwithf "invalid clr type spec %A" ast

let expandClrCall virt recur args =
    match args with
    | OnlyPositional [ objType; LiteralString name; Composite (OnlyPositional argTypes); returnType;
                       Composite (OnlyPositional args) ] ->
        let objType = clrTypeFromAst objType

        let argTypes =
            argTypes
            |> List.map (fun name -> clrTypeFromAst name)

        let returnType = clrTypeFromAst returnType
        let args = args |> List.map recur
        Atomic(ClrCall(objType, name, argTypes, returnType, virt, args))
    | _ -> failwithf "invalid std.clr.call (%A)" args

let expandClrNew recur args =
    match args with
    | OnlyPositional [ objType; Composite (OnlyPositional argTypes); Composite (OnlyPositional args) ] ->
        let objType = clrTypeFromAst objType
        let argTypes = argTypes |> List.map clrTypeFromAst
        let args = args |> List.map recur
        Atomic(ClrNew(objType, argTypes, args))
    | _ -> failwithf "invalid std.clr.new (%A)" args

let builtins: (Map<GlobalId, (AST -> ExpandedAST) -> Composite -> ExpandedAST>) =
    ([ { GlobalId.id = 7107475367237975794L }, expandClrCall false
       { GlobalId.id = 8698695841387214665L }, expandClrCall true
       { GlobalId.id = 6784770118985448820L }, expandClrNew
