    ...
    let rec aux (positionalIndex, tokens) =
        ...
        | token :: rest ->
            ...
            | Token t when t.s.StartsWith(":") ->
                ...
                let localIdToken = Token { t with s = "." + t.s.Substring(1) }
...
let makeComposite resolveName openingBracket children =
    let children = children |> List.filter (function | TokenTree.List _ -> true | Token t -> t.s <> ")")
...
let makeAST resolveName (token : TokenTree) : AST =
    match token with
    | TokenTree.List (openingBracket, children) ->
        makeComposite resolveName openingBracket children
    | Token t ->
