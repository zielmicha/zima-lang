...
let syntaxError (token : Token) msg =
    failwith msg
...
let makeAttrs resolveName (tokens : list<TokenTree>) : list<SymbolKey * TokenTree> =
    let rec aux (positionalIndex, tokens) =
        ...
        | Token t :: rest when t.s.EndsWith(":") ->
            ...
            | value :: rest ->
                let keyId = resolveName (t.s.Substring(0, t.s.Length - 1))
                Some ((GlobalId keyId, value), (positionalIndex, rest))
        | token :: rest ->
            match token with
            | Token t when t.s.StartsWith(":") ->
                let keyId = resolveName (t.s.Substring(1))
                let value = localIdToAst { LocalId.name = t.s.Substring(1) }
                Some ((GlobalId keyId, value), (positionalIndex + 1, rest))
            | _ ->
                Some ((Positional positionalIndex, token), (positionalIndex + 1, rest))
        | [] -> None
...
let makeComposite resolveName openingBracket children =
    let attrs = makeAttrs resolveName children |> List.map (fun (k, child) -> (k, makeAST resolveName child))
    // TODO: nodeId?
    // TODO: nice error message for duplicate keys
    compositeToAst { Composite.nodeId = 0L; attrs = Map.ofList attrs }

let unquoteString (s : string) =
    // TODO: unescape characters
    s.Substring(1, s.Length - 2)

let makeAST resolveName (token : TokenTree) =
    match token with
    ...
    | Token t ->
        if t.s.StartsWith(".") then
            ...
            stringLiteralToAst (unquoteString t.s)
        else
            let mutable i = -42L
            if System.Int64.TryParse(t.s, &i) then
                intLiteralToAst i
            else
                globalIdToAst (resolveName t.s)
